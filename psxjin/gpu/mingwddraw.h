...
// FOURCC codes for DirectX compressed texture pixel formats
...
// DDCOLORKEY
	...
	typedef struct _DDCOLORKEY
	{
		...
		DWORD       dwColorSpaceHighValue;  // High boundary of color space that is to
		// be treated as Color Key, inclusive
	} DDCOLORKEY;
	...
	typedef struct _DDBLTFX
	{
		...
		union
		{
			DWORD   dwZDestConst;                   // Constant to use as Z buffer for destination
			LPDIRECTDRAWSURFACE lpDDSZBufferDest;   // Surface to use as Z buffer for destination
		} DUMMYUNIONNAMEN(1);
		DWORD       dwZSrcConstBitDepth;            // Bit depth used to specify Z constant for source
		union
		{
			DWORD   dwZSrcConst;                    // Constant to use as Z buffer for source
			LPDIRECTDRAWSURFACE lpDDSZBufferSrc;    // Surface to use as Z buffer for source
		} DUMMYUNIONNAMEN(2);
		...
		union
		{
			DWORD   dwFillColor;                    // color in RGB or Palletized
...
#define DD_ROP_SPACE            (256/32)        // Space required to store ROP array
	...
	/*
	 * NOTE: Our chosen structure number scheme is to append a single digit to
	 ...
	 * by DirectDraw to interpret caps passed into DirectDraw by drivers written prior to the release of DirectDraw 2.0.
	...
	typedef struct _DDCAPS_DX1
	{
		...
		DWORD       dwAlignBoundaryDest;    // Destination rectangle alignment
		DWORD       dwAlignSizeDest;        // Destination rectangle byte size
		DWORD       dwAlignStrideAlign;     // Stride alignment
		DWORD       dwRops[DD_ROP_SPACE];   // ROPS supported
		DDSCAPS     ddsCaps;                // DDSCAPS structure has all the general capabilities
		DWORD       dwMinOverlayStretch;    // Minimum overlay stretch factor multiplied by 1000, EG 1000 == 1.0, 1300 == 1.3
		DWORD       dwMaxOverlayStretch;    // Maximum overlay stretch factor multiplied by 1000, EG 1000 == 1.0, 1300 == 1.3
		DWORD       dwMinLiveVideoStretch;  // OBSOLETE! This field remains for compatibility reasons only
		DWORD       dwMaxLiveVideoStretch;  // OBSOLETE! This field remains for compatibility reasons only
		DWORD       dwMinHwCodecStretch;    // OBSOLETE! This field remains for compatibility reasons only
		DWORD       dwMaxHwCodecStretch;    // OBSOLETE! This field remains for compatibility reasons only
	...
	/*
	 * This structure is the DDCAPS structure as it was in version 2 and 3 of DirectX.
	...
	typedef struct _DDCAPS_DX3
	{
		...
		DWORD       dwReserved2;            // Reserved
		DWORD       dwReserved3;            // Reserved
		DWORD       dwSVBCaps;              // Driver-specific capabilities for system to video memory blts
		DWORD       dwSVBCKeyCaps;          // Driver color key capabilities for system to video memory blts
		DWORD       dwSVBFXCaps;            // Driver FX capabilities for system to video memory blts
		DWORD       dwSVBRops[DD_ROP_SPACE]; // ROPS supported for system to video memory blts
		DWORD       dwVSBCaps;              // Driver-specific capabilities for video memory to system blts
		DWORD       dwVSBCKeyCaps;          // Driver color key capabilities for video memory to system blts
		DWORD       dwVSBFXCaps;            // Driver FX capabilities for video memory to system blts
		DWORD       dwVSBRops[DD_ROP_SPACE]; // ROPS supported for video memory to system blts
		DWORD       dwSSBCaps;              // Driver-specific capabilities for system to system blts
		DWORD       dwSSBCKeyCaps;          // Driver color key capabilities for system to system blts
		DWORD       dwSSBFXCaps;            // Driver FX capabilities for system to system blts
		DWORD       dwSSBRops[DD_ROP_SPACE]; // ROPS supported for system to system blts
	...
	/*
	 * This structure is the DDCAPS structure as it was in version 5 of DirectX.
...
// Members added for DX5:
		...
		/*138*/
		DWORD   dwSVBCaps2;             // More driver-specific capabilities for system to video memory blts
		/*13c*/
		DWORD   dwNLVBCaps;               // Driver-specific capabilities for non-local to local video memory blts
		/*140*/
		DWORD   dwNLVBCaps2;              // More driver-specific capabilities non-local to local video memory blts
		/*144*/
		DWORD   dwNLVBCKeyCaps;           // Driver color key capabilities for non-local to local video memory blts
		/*148*/
		DWORD   dwNLVBFXCaps;             // Driver FX capabilities for non-local to local blts
		/*14c*/
		DWORD   dwNLVBRops[DD_ROP_SPACE]; // ROPS supported for non-local to local blts
	} DDCAPS_DX5;
	...
	typedef struct _DDCAPS_DX6
	{
		...
		DWORD   dwRops[DD_ROP_SPACE];   // ROPS supported
		/* 84*/
		DDSCAPS ddsOldCaps;             // Was DDSCAPS  ddsCaps. ddsCaps is of type DDSCAPS2 for DX6
		/* 88*/
		DWORD   dwMinOverlayStretch;    // Minimum overlay stretch factor multiplied by 1000, EG 1000 == 1.0, 1300 == 1.3
		/* 8c*/
		DWORD   dwMaxOverlayStretch;    // Maximum overlay stretch factor multiplied by 1000, EG 1000 == 1.0, 1300 == 1.3
		...
		/*14c*/
		DWORD   dwNLVBRops[DD_ROP_SPACE]; // ROPS supported for non-local to local blts
// Members added for DirectX 6 release
		/*16c*/
...
// DDPIXELFORMAT
	...
	typedef struct _DDOVERLAYFX
	{
		...
		union
		{
			DWORD   dwAlphaDestConst;               // Constant to use as alpha channel for destination
			LPDIRECTDRAWSURFACE lpDDSAlphaDest;     // Surface to use as alpha channel for destination
		} DUMMYUNIONNAMEN(1);
		DWORD       dwAlphaSrcConstBitDepth;        // Bit depth used to specify alpha constant for source
		union
		{
			DWORD   dwAlphaSrcConst;                // Constant to use as alpha channel for source
			LPDIRECTDRAWSURFACE lpDDSAlphaSrc;      // Surface to use as alpha channel for source
		} DUMMYUNIONNAMEN(2);
...
#define MAX_DDDEVICEID_STRING           512
	...
	typedef struct tagDDDEVICEIDENTIFIER
	{
		 ...
		 * re-profile the graphics subsystem
...
// Flags for the IDirectDraw4::GetDeviceIdentifier method
	 ...
	 * host adapter, but is typically physically located on a separate card. The stacked secondary information is
...
// DDSURFACEDESC
	...
	typedef struct _DDSURFACEDESC
	{
		...
		DDCOLORKEY          ddckCKDestBlt;          // Color key for destination blitter use
		DDCOLORKEY          ddckCKSrcOverlay;       // Color key for source overlay use
		DDCOLORKEY          ddckCKSrcBlt;           // Color key for source blitter use
...
// DDSURFACEDESC2
	...
	typedef struct _DDSURFACEDESC2
	{
		...
		DDSCAPS2            ddsCaps;                // DirectDraw surface capabilities
...
// The surface's optimized pixel format is compressed

#define DDOSDCAPS_OPTCOMPRESSED                 0x00000001l

// The surface's optimized pixel format is re-ordered

#define DDOSDCAPS_OPTREORDERED                  0x00000002l

// The opt surface is a monolithic mip-map
...
#define DDSCAPS_FRONTBUFFER                     0x00000020l
	...
	/*
	 * Indicates that this surface is any off screen surface that is not an overlay,
	 * texture, z buffer, front buffer, back buffer, or alpha surface.  It is used
...
#define DDSCAPS2_HARDWAREDEINTERLACE            0x00000002L
	 ...
	 * (for procedural textures, dynamic light maps, etc). Surfaces with this cap
...
#define DDSCAPS2_HINTDYNAMIC                    0x00000004L
	...
	/*
	 * Indicates to the driver that this surface can be re-ordered/re-tiled on
...
#define DDSCAPS2_RESERVED2                      0x00000040L
	 ...
	 * The driver is free to optimize this surface via re-tiling and actual compression.
...
#define DDSCAPS2_OPAQUE                         0x00000080L
	 ...
	 * intend to use anti-aliasing. Only valid if DDSCAPS_3DDEVICE is also set.
	 */
...
#define DDSCAPS2_CUBEMAP                        0x00000200L
	 ...
	 * - After creation, each face in the cube map will have exactly one of these
	 *   bits set.
	 */
...
#define DDSCAPS2_CUBEMAP_ALLFACES ( DDSCAPS2_CUBEMAP_POSITIVEX |\
	...
	/*
	 * This flag is an additional flag which is present on mipmap sub levels from DX7 onwards
...
#define DDCAPS_3D                       0x00000001l
	...
	/*
	 * Indicates that DirectDraw will support only destination rectangles that are aligned
...
// Indicates that palette changes can be synced with the vertical refresh
...
// Color key is hardware assisted (DDCAPS_COLORKEY will also be set)
...
// Display hardware is capable of color fill with blitter
...
// Display hardware is capable of depth filling Z-buffers with blitter

#define DDCAPS_BLTDEPTHFILL             0x10000000l

// Display hardware is capable of clipping while blitting
...
#define DDCAPS2_NONLOCALVIDMEM          0x00000200l
	...
	/*
	 * Driver supports non-local video memory but has different capabilities for
...
#define DDCAPS2_NONLOCALVIDMEMCAPS      0x00000400l
	...
	/*
	 * Driver neither requires nor prefers surfaces to be page locked when performing
	 * blitters involving system memory surfaces
	 */
...
// Driver supports blitting any FOURCC surface to another surface of the same FOURCC
...
#define DDCAPS2_CANMANAGETEXTURE        0x00800000l
	 ...
	 * put managed surfaces in non-local video memory. Drivers that cannot texture from
	 * local video memory SHOULD NOT set this cap.
	 */
...
#define DDCAPS2_STEREO                  0x02000000L
	 ...
	 *  all the associated system memory blitter caps must be correct).
	 *  It implies that the system to video blitter caps in DDCAPS also apply to system to
	 *  nonlocal blitters. IE the dwSVBCaps, dwSVBCKeyCaps, dwSVBFXCaps and dwSVBRops
	 *  members of DDCAPS (DDCORECAPS) are filled in correctly.
	 *  Any blitter from system to nonlocal memory that matches these caps bits will
	 *  be passed to the driver.
	 *
	 * NOTE: This is intended to enable the driver itself to do efficient re-ordering
...
// DIRECTDRAW FX CAPABILITY FLAGS
	...
	/*
	 * Uses arithmetic operations to stretch and shrink surfaces during blitter
...
#define DDFXCAPS_BLTARITHSTRETCHY       0x00000020l
	...
	/*
	 * Uses arithmetic operations to stretch during blitter
...
// Supports mirroring left to right in blitter

#define DDFXCAPS_BLTMIRRORLEFTRIGHT     0x00000040l

// Supports mirroring top to bottom in blitter

#define DDFXCAPS_BLTMIRRORUPDOWN        0x00000080l

// Supports arbitrary rotation for blitter

#define DDFXCAPS_BLTROTATION            0x00000100l

// Supports 90 degree rotations for blitter

#define DDFXCAPS_BLTROTATION90          0x00000200l

	/*
	 * DirectDraw supports arbitrary shrinking of a surface along the
	 * x axis (horizontal direction) for blitter.
	 */
...
#define DDFXCAPS_BLTSHRINKX             0x00000400l
	 ...
	 * along the x axis (horizontal direction) for blitter.
	 */
...
#define DDFXCAPS_BLTSHRINKXN            0x00000800l
	...
	/*
	 * DirectDraw supports arbitrary shrinking of a surface along the
	 * y axis (horizontal direction) for blitter.
	 */
...
#define DDFXCAPS_BLTSHRINKY             0x00001000l
	...
	/*
	 * DirectDraw supports integer shrinking (1x,2x,) of a surface
	 * along the y axis (vertical direction) for blitter.
	 */
...
// Driver can do alpha blending for blitters

#define DDFXCAPS_BLTALPHA               0x00000001l

// Driver can do surface reconstruction filtering for warped blitters
...
#define DDCKEY_COLORSPACE       0x00000001l
	 ...
	 * used as a destination color key for blitter operations.
	 */
...
#define DDCKEY_DESTOVERLAY      0x00000004l
	...
	/*
	 * Set if the structure specifies a color key or color space which is to be
	 * used as a source color key for blitter operations.
	 */
...
// DIRECTDRAW COLOR KEY CAPABILITY FLAGS
	...
	/*
	 * Supports transparent blitting using a color key to identify the replaceable
...
#define DDCKEYCAPS_DESTBLT                      0x00000001l
	...
	/*
	 * Supports transparent blitting using a color space to identify the replaceable
	 * bits of the destination surface for RGB colors.
	 */
...
#define DDCKEYCAPS_DESTBLTYUV                   0x00000008l
	...
	/*
	 * Supports overlaying using color keying of the replaceable bits of the surface
...
#define DDCKEYCAPS_DESTOVERLAYONEACTIVE         0x00000080l
	...
	/*
	 * Supports overlaying using color keying of the replaceable bits of the
...
#define DDCKEYCAPS_DESTOVERLAYYUV               0x00000100l
	...
	/*
	 * Supports transparent blitting using the color key for the source with
...
#define DDCKEYCAPS_SRCBLT                       0x00000200l
	...
	/*
	 * Supports transparent blitting using a color space for the source with
	 * this surface for RGB colors.
	 */
...
// There are no bandwidth trade-offs for using color key with an overlay
...
#define DDPF_STENCILBUFFER                      0x00004000l
	...
	/*
	 * Pre-multiplied alpha format -- the color components have been
...
#define DDPF_LUMINANCE                          0x00020000l
	...
	/*
	 * Luminance data in the pixel format is valid.
	 * Use this flag when hanging luminance off bump map surfaces,
...
#define DDSCL_ALLOWREBOOT                       0x00000002l
	 ...
	 * Prevents DDRAW from minimizing/restoring the application window on activation
	 */
	 
#define DDSCL_NOWINDOWCHANGES                   0x00000004l

// Application wants to work as a regular Windows application

#define DDSCL_NORMAL                            0x00000008l

// Application wants exclusive access

#define DDSCL_EXCLUSIVE                         0x00000010l

// Application can deal with non-windows display modes
...
#define DDSCL_SETDEVICEWINDOW                   0x00000100l
	...
	/*
	 * Application wants DDRAW to create a window to be associated with the
	 * DDRAW object
	 */
...
#define DDSCL_CREATEDEVICEWINDOW                0x00000200l
	...
	/*
	 * Application explicitly asks DDRAW/D3D to be multi-thread safe. This makes D3D
...
#define DDSCL_MULTITHREADED                     0x00000400l
	...
	/*
	 * Application specifies that it would like to keep the FPU set up for optimal Direct3D
...
#define DDSCL_FPUSETUP                          0x00000800l
	...
	/*
	 * Application specifies that it needs either double precision FPU or FPU exceptions
	 * enabled. This makes Direct3D explicitly set the FPU state each time it is
...
// DIRECTDRAW BLT FLAGS
	 ...
	 * attached to the destination surface as the alpha channel for this blitter.
	 */
...
#define DDBLT_ALPHADEST                         0x00000001l
	 ...
	 * for the destination surface for this blitter.
	 */
...
#define DDBLT_ALPHADESTNEG                      0x00000004l
	 ...
	 * channel for the destination for this blitter.
	 */
...
#define DDBLT_ALPHAEDGEBLEND                    0x00000010l
	...
	/*
	 * Use the alpha information in the pixel format or the alpha channel surface
	 * attached to the source surface as the alpha channel for this blitter.
	 */
...
#define DDBLT_ALPHASRC                          0x00000020l
	 ...
	 * for the source for this blitter.
	 */
...
#define DDBLT_ALPHASRCSURFACEOVERRIDE           0x00000100l
	...
	/*
	 * Do this blitter asynchronously through the FIFO in the order received.  If
...
#define DDBLT_KEYSRCOVERRIDE                    0x00010000l
	 ...
	 * for this blitter.  These ROPs are the same as the ones defined in the Win32 API.
	 */
...
#define DDBLT_ROTATIONANGLE                     0x00040000l
	...
	/*
	 * Z-buffered blitter using the Z-buffers attached to the source and destination
...
#define DDBLT_ZBUFFER                           0x00080000l
	...
	/*
	 * Z-buffered blitter using the dwConstDest Z field and the dwZBufferOpCode field
...
#define DDBLT_ZBUFFERDESTCONSTOVERRIDE          0x00100000l
	...
	/*
	 * Z-buffered blitter using the lpDDSDestZBuffer field and the dwZBufferOpCode
...
#define DDBLT_ZBUFFERDESTOVERRIDE               0x00200000l
	...
	/*
	 * Z-buffered blitter using the dwConstSrcZ field and the dwZBufferOpCode field
	 * in the DDBLTFX structure as the Z-buffer and Z-buffer opcode respectively
	 * for the source.
	 */
...
#define DDBLT_ZBUFFERSRCCONSTOVERRIDE           0x00400000l
	...
	/*
	 * Z-buffered blitter using the lpDDSSrcZBuffer field and the dwZBufferOpCode
	 * field in the DDBLTFX structure as the Z-buffer and Z-buffer opcode
...
#define DDBLT_ZBUFFERSRCOVERRIDE                0x00800000l
	...
	/*
	 * Wait until the device is ready to handle the blitter
	 * This will cause blitter to not return DDERR_WASSTILLDRAWING
	 */
...
#define DDFLIP_ODD                           0x00000004L
	 ...
	 * not check to see if the physical flip has finished (IE will not return
...
// Add a dirty rectangle to an emulated overlayed surface

#define DDOVER_ADDDIRTYRECT                     0x00008000l

// Re-draw all dirty rectangles on an emulated overlayed surface

#define DDOVER_REFRESHDIRTYRECTS                0x00010000l

// Re-draw the entire surface on an emulated overlayed surface
...
// Auto flip the overlay when ever the video port auto flips
...
// If stretching, use arithmetic stretching along the Y-axis for this blitter

#define DDBLTFX_ARITHSTRETCHY                   0x00000001l

	/*
	 * Do this blitter mirroring the surface left to right.  Spin the
...
#define DDBLTFX_MIRRORLEFTRIGHT                 0x00000002l
	...
	/*
	 * Do this blitter mirroring the surface up and down.  Spin the surface
	 * around its X-axis.
	 */
	 
#define DDBLTFX_MIRRORUPDOWN                    0x00000004l

// Schedule this blitter to avoid tearing

#define DDBLTFX_NOTEARING                       0x00000008l

// Do this blitter rotating the surface one hundred and eighty degrees

#define DDBLTFX_ROTATE180                       0x00000010l

// Do this blitter rotating the surface two hundred and seventy degrees

#define DDBLTFX_ROTATE270                       0x00000020l

// Do this blitter rotating the surface ninety degrees

#define DDBLTFX_ROTATE90                        0x00000040l

	/*
	 * Do this Z blitter using dwZBufferLow and dwZBufferHigh as range values
...
#define DDBLTFX_ZBUFFERRANGE                    0x00000080l
	...
	/*
	 * Do this Z blitter adding the dwZBufferBaseDest to each of the sources Z values
	 * before comparing it with the destination Z values.
	 */
...
// Is it OK to blit now?

#define DDGBS_CANBLT                    0x00000001l

// Is the blitter to the surface finished?
...
#define DDERR_INVALIDCAPS                       MAKE_DDHRESULT( 100 )
	...
	/*
	 * DirectDraw does not support provided Clip list
	 */
...
// No source color key specified for this operation
...
#define DDERR_VERTICALBLANKINPROGRESS           MAKE_DDHRESULT( 537 )
	...
	/*
	 * Informs DirectDraw that the previous blitter which is transferring information
...
// Rectangle provided was not horizontally aligned on registered boundary
...
#define DDERR_REGIONTOOSMALL                    MAKE_DDHRESULT( 566 )
	...
	/*
	 * An attempt was made to set a clip list for a clipper object that
...
#define DDERR_NOHWND                            MAKE_DDHRESULT( 569 )
	...
	/*
	 * HWND used by DirectDraw CooperativeLevel has been sub-classed,
...
// Attempt was made to set a palette on a mipmap sub-level
...
// Direct3D has not yet been initialized
