...
public:
    ...
    void setParameters(uint sampleRate,                             // Sample rate of sound being processed (measured in Hertz)
                       uint sequenceMS = DEFAULT_SEQUENCE_MS,       // Single processing sequence length (measured in milliseconds)
                       uint seekwindowMS = DEFAULT_SEEKWINDOW_MS,   // Offset seeking window length (measured in milliseconds)
                       uint overlapMS = DEFAULT_OVERLAP_MS          // Sequence overlapping length (measured in milliseconds)
                       );
