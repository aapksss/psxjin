...
double TDStretchSSE::calcCrossCorrStereo(const float *pV1, const float *pV2) const
{
    ...
    // This can mean up to a 10 fold difference (including part of which is
    ...
    */
    ...
    /* This is corresponding routine in assembler. This may be slightly faster
       than intrinsic version, but more difficult to maintain and get compiled on multiple
       platforms.
    ...
    _asm 
    {
        ...
        // Give prefetch hints to the CPU of what data is needed soon
    ...
    loop1:
        prefetcht0 [eax + 64]     // Give a prefetch hint to the CPU as to what data is needed soon
        prefetcht0 [ebx + 32]     // Give a prefetch hint to the CPU as to what data is needed soon
        movups  xmm1, [eax]
        mulps   xmm1, [ebx]
        addps   xmm0, xmm1
        ...
        prefetcht0 [eax + 96]     // Give a prefetch hint to the CPU as to what data is needed soon
        prefetcht0 [ebx + 64]     // Give a prefetch hint to the CPU as to what data is needed soon
        ...
        movhlps xmm1, xmm0          // Move 3 and 4 of xmm0 to 1 and 2 of xmm1
        addps   xmm1, xmm0
        movaps  xmm2, xmm1
...
void FIRFilterSSE::setCoefficients(const float *coeffs, uint newLength, uint uResultDivFactor)
{
    ...
    // Also re-arrange coefficients suitably for 3DNow!
...
uint FIRFilterSSE::evaluateFilterStereo(float *dest, const float *source, uint numSamples) const
{
    ...
    for (j = 0; j < count; j += 2)
    {
        ...
        pFil = (__m128*)filterCoeffsAlign;  // Filter coefficients. Note: assumes coefficients
                                            // These are aligned to 16-byte boundary
        sum1 = sum2 = _mm_setzero_ps();

        for (i = 0; i < length / 8; i ++) 
        {
            // Unroll loop for efficiency and calculate filter for 2*2 stereo samples 
            // at each pass
        ...
        // Post-shuffle and add the filtered values and store to destination
	...
	// Implement this ASAP if possible
    // Ideas for further improvement:
    // 1. If it could be guaranteed that source were always aligned to 16-byte 
    //    boundary, a faster aligned _mm_load_ps instruction could be used
    // 2. If it could be guaranteed that destination were always aligned to 16-byte 
    ...
    */
    ...
    _asm
    {
        // Very important note: data in src must be aligned to 
        // 16-byte boundary
        mov     edx, count
    ...
    loop2:
        ...
        prefetcht0 [esi + 32]     // Give a prefetch hint to the CPU as to what data is needed soon
        prefetcht0 [edi + 32]     // Give a prefetch hint to the CPU as to what data is needed soon
        ...
        prefetcht0 [esi + 64]     // Give a prefetch hint to the CPU as to what data is needed soon
        prefetcht0 [edi + 64]     // Give a prefetch hint to the CPU as to what data is needed soon
