...
RateTransposer::RateTransposer() : FIFOProcessor(&outputBuffer)
{
    ...
    // Instantiates the anti-aliasing filter with default tap length
    // of 32
...
// Returns non-zero if anti-aliasing filter is enabled
...
void RateTransposer::setRate(float newRate)
{
    ...
    // Design a new anti-aliasing filter
...
// Returns the number of samples returned in the destination buffer
...
void RateTransposer::setChannels(const uint numchannels)
{
    ...
    // Initializes the linear interpolation registers
...
RateTransposerInteger::RateTransposerInteger() : RateTransposer()
{
    ...
    // versions (<master yoda>peculiar C++ can be</my>) < What is this message about?
...
RateTransposerFloat::RateTransposerFloat() : RateTransposer()
{
    // Call these here as these are virtual functions; calling these
    // from the base class constructor wouldn't execute the overloaded
    // versions (<master yoda>peculiar C++ can be</my>) < Again, what is this even talking about?
    resetRegisters();
    setRate(1.0f);
}
