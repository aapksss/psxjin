...
public:
	...
	std::string GetType();
	void SetMenuID(int menuID);		 // Set the menu item that the recent sub-menu will be created on
	...
	void MakeRecentMenu(HINSTANCE instance);	// Creates the sub-menu using the HINSTANCE of the main app
	...
	// To do: GetRecentItems - returns a vector of strings, for emulators that don't save to ini

	void FlipAutoLoad();
	bool GetAutoLoad();

private:
	int BaseID;					// The user will decide an ID for which to build the menu items from (one that doesn't clash with other resource file items!)
	int ClearID;				// Menu item for the clear list (will be one baseID + MAX)
	int menuItem;				// Which menu item to build the recent sub-menu off	
	HWND GhWnd;					// Handle to the Windows GUI
	std::string rtype;			// Type of recent files, will be used when saving to ini files
	std::string autoloadstr;	// Name of auto load parameter saved to ini "Autoload" + rtype
	bool autoload;				// For auto-loading the most recent item
	
	std::vector<std::string> RecentItems;	 // The list of recent filenames
	HMENU recentmenu;						 // Handle to the recent ROMs sub-menu
